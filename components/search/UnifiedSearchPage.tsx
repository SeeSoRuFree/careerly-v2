...
interface UnifiedSearchPageProps {
  /** URL에서 가져온 sessionId (세션 조회 모드) */
  initialSessionId?: string;
}

export function UnifiedSearchPage({ initialSessionId }: UnifiedSearchPageProps) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const query = searchParams.get('q') || '';
  const openLoginModal = useStore((state) => state.openLoginModal);

  // 모드 결정: sessionId가 있고 query가 없으면 세션 조회 모드
  const isSessionMode = !!initialSessionId && !query;
  ...
  const { data: fetchedSession, isLoading: isSessionLoading, error: sessionError } = useChatSessionWithFallback(
    isSessionMode ? initialSessionId! : '',
    {
  ...
  // 세션 상태 (불완전 세션 감지용)
  const [isIncompleteSession, setIsIncompleteSession] = useState(false);

  // 세션 데이터가 로드되면 상태 업데이트
  useEffect(() => {
    if (fetchedSession && isSessionMode) {
      const userMessage = fetchedSession.messages.find(m => m.role === 'user');
      const assistantMessage = fetchedSession.messages.find(m => m.role === 'assistant');

      // 세션 상태 확인 (status가 'streaming' 또는 'failed'인 경우 불완전 세션)
      const isIncomplete = fetchedSession.status === 'streaming' || fetchedSession.status === 'failed';
      setIsIncompleteSession(isIncomplete);

      if (userMessage && assistantMessage) {
        setSessionData({
          query: userMessage.content,
          answer: assistantMessage.content,
          sources: assistantMessage.sources?.all || [],
          createdAt: fetchedSession.created_at,
          isPublic: fetchedSession.is_public,
          sharedPostId: fetchedSession.shared_post_id,
          messageId: assistantMessage.id,
          currentFeedback: assistantMessage.is_liked,
        });
      } else if (isIncomplete && userMessage) {
        // 불완전 세션이지만 사용자 메시지는 있는 경우
        setSessionData({
          query: userMessage.content,
          answer: '',
          sources: [],
  ...
  const handleShareToCommunity = () => {
    // 공유 성공 시 세션 데이터 리프레시
    // sessionData 업데이트 (세션 모드일 때)
    if (sessionData) {
      setSessionData(prev => prev ? { ...prev, sharedPostId: -1 } : null); // 임시 값, 실제로는 API 응답에서 받아옴
    }
  };
  ...
  const handleRetry = () => {
    const queryToUse = sessionData?.query || query;
    if (queryToUse) {
      // 불완전 세션 상태 초기화
      setIsIncompleteSession(false);
      // 같은 질문으로 새로운 스트리밍 시작
      startStreaming(queryToUse, null);
    }
  };
  ...
  // 표시할 데이터 결정
  const displayQuery = sessionData?.query || query;
  const displayAnswer = streamingContent || completedAnswer || sessionData?.answer || '';
  const displaySources = streamingSources.length > 0
    ? streamingSources
    : completedSources.length > 0
      ? completedSources
      : sessionData?.sources || [];
  const displayMessageId = completedMessageId || sessionData?.messageId;
  const displayCurrentFeedback = sessionData?.currentFeedback;
  const isPublic = sessionData?.isPublic || false;
  const sharedPostId = sessionData?.sharedPostId;
  const isSharedToCommunity = !!sharedPostId;
  ...
  // 세션 조회 모드: 에러 상태
  if (isSessionMode && (sessionError || !fetchedSession)) {
    const isAuthError = sessionError?.message?.includes('401') || sessionError?.message?.includes('인증');
    const isNotFound = sessionError?.message?.includes('404') || sessionError?.message?.includes('찾을 수 없');
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className={cn(
                  ...
                  {sessionError?.message || '세션 정보를 불러올 수 없습니다.'}
                </p>
                <button
                  onClick={() => router.push('/')}
                  className={cn(
  ...
  // 쿼리도 없고 세션 데이터도 없는 경우
  if (!displayQuery && !isSessionMode) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
  ...
  const hasCompletedContent = !!completedAnswer || !!sessionData?.answer;
      ...
      <div className="container mx-auto px-4 py-4 max-w-4xl">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4 pb-3 border-b border-slate-200">
          <div className="flex items-center gap-3">
            ...
            {/* 세션 정보 표시 (세션 모드일 때) */}
            {isSessionMode && createdAt && !isStreaming && (
        ...
        {/* 불완전 세션 경고 */}
        {isIncompleteSession && !error && !isStreaming && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-amber-700 mb-3">
                  {fetchedSession?.status === 'failed'
                    ? '답변 생성에 실패했습니다. 다시 시도해주세요.'
                    : '네트워크 문제나 예상치 못한 오류로 답변 생성이 중단되었습니다.'}
                </p>
                <button
                  onClick={handleRetry}
                  className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 rounded-md transition-colors"
                >
        ...
        <div className="space-y-6">
          {viewMode === 'answer' && (
            <>
              ...
              {/* 커뮤니티 공유 CTA (완료 후에만 표시) */}
              {!isStreaming && hasCompletedContent && sessionId && (
          ...
          {/* Sources 모드 */}
          {viewMode === 'sources' && !isStreaming && hasCompletedContent && (
